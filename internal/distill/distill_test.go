...
func TestRunParsesWellFormedProposal(t *testing.T) {
	...
	prop, err := Run(context.Background(), fake, claudecode.Digest{Text: "[user] hi"}, fixedOpts())
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRunSurvivesMarkdownFenceAndDropsBadItems(t *testing.T) {
	...
	prop, err := Run(context.Background(), fake, claudecode.Digest{Text: "x"}, fixedOpts())
	if err != nil {
		t.Fatalf("Run: %v", err)
	}
...
func TestRunRejectsNonJSON(t *testing.T) {
	...
	if _, err := Run(context.Background(), fake, claudecode.Digest{Text: "x"}, fixedOpts()); err == nil {
